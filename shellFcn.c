...
void printPromt(char* promptString) {
...
void runCmd(char* path, char** paths, char* cmd, char** args, char** envp, char mode) {
    int cmdExists = getCmdPath(path, paths, cmd, args); //find command paths

    if (cmdExists) { //cmd exists
        if (mode == '&') {
            executeCmd(cmd, args, envp, 1);
        }
        else {
            executeCmd(cmd, args, envp, 0);
        }
